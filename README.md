...
*NOTE*: A _number_ is a real (floating-point) value: arithmetic and comparisons use real-number (not integer) semantics. For example, `0.9 > 0.1` is true, and `jgz` with `0.5` skips the next instruction. Unless stated otherwise, supplying an invalid argument type (e.g. a non-string context key, or a non-number where a number is required) results in an error.
...
| `+` / `plus` | B, A                                     | number, number               | Adds A to B and pushes the result onto the stack. Both operands must be numbers; otherwise an error is raised.                                                                                                                                                                                                                                                                    |
| `-` / `min`  | B, A                                     | number, number               | Subtracts B from A and pushes the result onto the stack. Both operands must be numbers; otherwise an error is raised.                                                                                                                                                                                                                                                             |
| `*` / `mul`  | B, A                                     | number, number               | Multiplies B and A and pushes the result onto the stack. Both operands must be numbers; otherwise an error is raised.                                                                                                                                                                                                                                                             |
...
| `randInt`    | A                                        | number                       | Generate a random integer `N = floor(random() * A)` and push it. For `A > 0` this yields `0 <= N < A`; for `A = 0` the result is `0`. `A` must be a number.                                                                                                                                                                                                                     |
| `charCode`   | A                                        | number                       | Pops A (a number) and pushes the string consisting of the single Unicode character whose code point is `A mod 65536` (equivalent to JavaScript's `String.fromCharCode(A)`). A must be a number.                                                                                                                                                                                  |
| `ppc`        |                                          |                              | Push the current program counter onto the stack                                                                                                                                                                                                                                                                                                                                  |
| `not`        | A                                        | number                       | Pops A off the stack. If A is 0, push 1 onto the stack. Otherwise, push 0 onto the stack. A must be a number.                                                                                                                                                                                                                                                                    |
| `or`         | B, A                                     | number, number               | If both A and B are 0, push 0 onto the stack. Otherwise, if either is nonzero, push 1 onto the stack. A and B must be numbers.                                                                                                                                                                                                                                                  |
| `and`        | B, A                                     | number, number               | If either A or B is 0, push 0 onto the stack. Otherwise, if both are nonzero, push 1 onto the stack. A and B must be numbers.                                                                                                                                                                                                                                                   |
| `jgz`        | A                                        | number                       | If A (a number) is greater than zero, skip ("hop over") the next instruction. Fractional values are compared as real numbers, so e.g. `0.5` skips.                                                                                                                                                                                                                              |
| `jz`         | A                                        | number                       | If A (a number) is zero, skip ("hop over") the next instruction. Values are compared as real numbers.                                                                                                                                                                                                                                                                            |
| `gt`         | B, A                                     | number, number               | If A is greater than B, push 1 onto the stack. Otherwise, push 0. A and B must be numbers; compared as real numbers.                                                                                                                                                                                                                                                             |
| `lt`         | B, A                                     | number, number               | If A is less than B, push 1 onto the stack. Otherwise, push 0. A and B must be numbers; compared as real numbers.                                                                                                                                                                                                                                                                |
| `eq`         | B, A                                     | string/number, string/number | If A and B are equal (equal value if number, byte-for-byte equal if string), push 1 onto the stack. Otherwise, push 0. Numbers are compared by value, so `1` and `1.0` are equal; a number and a string are never equal.                                                                                                                                                     |
...
| `getContext` | A                                        | string                       | Pops A off the stack, then gets the context value pointed to by A and pushes it onto the stack. A must be a string; an error is raised if A is not a string or if no value is set for A.                                                                                                                                                                                      |
| `hasContext` | A                                        | string                       | Pops A off the stack, then checks if the context has any existing value for A; if so, pushes 1 to the stack, 0 otherwise. A must be a string; an error is raised if A is not a string.                                                                                                                                                                                        |
| `setContext` | B, A                                     | string/number, string        | Pops A and B off the stack. The context value pointed to by A (the key, which must be a string) is set to B (a string or number). An error is raised if A is not a string.                                                                                                                                                                    |
| `delContext` | A                                        | string                       | Pops A off the stack, then deletes the value (and key / pointer, depending on implementation) pointed to by A completely, effectively freeing it. A must be a string; an error is raised if A is not a string.                                                                                                                                                                |
...
| `stdout`     | A                                        | string/number                | Pops A off the stack. If A is a string, it is written; if A is a number, it is coerced to a string exactly as JavaScript does (`"" + A`): whole numbers print without a decimal part (e.g. `1`, not `1.000000`), `-0` prints as `0`, and `Infinity`/`NaN` print as such. Popping from an empty stack writes `undefined`.                                                      |
...
Number literals may be integers _or_ decimals, e.g. `1`, `-5` and `0.5` are all valid; they are parsed as real (floating-point) numbers.
